...
Item {
   ...
   // Add rpm property with random value
    property int rpm: 0

    // Add speed property that calculates based on rpm
    property int speed: calculateSpeed(rpm)

    // Add function to calculate speed based on rpm
    function calculateSpeed(rpm) {
        return Math.round(rpm / 10)
    }

    // Add timer to update rpm randomly
    Timer {
        interval: 3000  // Update every second
        running: true
        repeat: true
        onTriggered: {
            // Generate random RPM between 0 and 450
            valueSource.rpm = Math.floor(Math.random() * 450)
        }
    }
